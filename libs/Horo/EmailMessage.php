...
class EmailMessage {
	...
	public function fetch() {
		...
		if(!$structure) {
			...
			#print_r($structure);
			if(!$structure->parts) {
				$content = imap_body($this->connection, $this->messageNumber);
				if($content) {
					$this->bodyHTML = $this->bodyPlain = Sanity::iconvDecode($charset, Sanity::doEncoding($structure->encoding, $content ));
				}
			} else {
		...
		foreach($messageParts as $part) {
			...
			if($part->type == 0) {
				if($part->subtype == 'PLAIN') {
					...
					$this->bodyPlain .= Sanity::iconvDecode($charset, Sanity::doEncoding($part->encoding, $this->getPart($partNumber, $part->encoding) ));

					}
				else {
					#$this->bodyHTML .= $this->getPart($partNumber, $part->encoding);
					$this->bodyHTML .= Sanity::iconvDecode($charset, Sanity::doEncoding($part->encoding, $this->getPart($partNumber, $part->encoding) ));
				}
			}
			elseif($part->type == 2) {
