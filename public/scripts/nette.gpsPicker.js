...
var GpsPicker = function () {
	...
	this.initialize = function (el, options) {
		...
		$inputs.live('change', function() {
			latlng = new google.maps.LatLng(latInput.val(), lngInput.val());
		...
		return $el.data('gpspicker', $.extend({
			map: map
		}, new handlers[options.shape]($el, $inputs, map, marker, options) || {}));
	};
...
// single GPS position
GpsPicker.registerHandler('point', function ($el, $inputs, map, marker, options) {
	var $latInput = $inputs.filter('[id$=lat]');
	var $lngInput = $inputs.filter('[id$=lng]');
